...
```diff
  const path = require('path');
  const HtmlWebpackPlugin = require('html-webpack-plugin');
+ const {CleanWebpackPlugin} = require('clean-webpack-plugin');//CleanWebpackPlugin3.0以上版本需要对CleanWebpackPlugin进行解构，沿用原老方法会报错
  ...
  module.exports = {
    ...
    plugins: [
+     new CleanWebpackPlugin(),//不要添加任何参数，否则报错
      new HtmlWebpackPlugin({
        title: 'Output Management'
      })
    ],
